//2021 by amanda & SmartApe, updated 2021/July/05 16:33:67, updated 2021, July, 29 10:22
...
ok_ar:;
     ...
     do
     {
	       ...
	       if ('.' != valquiria_wide_to_utf8(ffd.cFileName)[0])
	       {
		    ...
		    if (recurse_on_subfolders_arp)
		    {
			 ...
			 if (0 == skip_ar)
			 {
			      ...
			      if (true == only_get_number_of_files_ar)
			      {
				   amanda_itens++;
			      }
			      else
			      {
				   ...
				   {
					dump_file_or_folder_iso(final_file_or_folder_ar, fixo_path_ar);
				   }

				   ricard0_itens_processed++;
			      }

			      EnumerateFolder(szDir, 0, only_get_number_of_files_ar);
			 }
		    }
	       }
	  }
	  else
	  {
	       ...
	       {
		    static char temp_ar[1024] = { 0 };
		    snprintf(temp_ar, 600, "Skipping file %s because the path is too large for windows (above MAX_PATH)", temp_2_ar);
		    skip_ar = 1;
		    add_more_one(temp_ar);
		    warning_flag = 28;
		    files_count++;
		    ...
		    if (true == only_get_number_of_files_ar)
		    {
			 ...
			 strtolower_ar(my_copy_of_filename_ar);
			 {
			      ...
			      if (achei_o_match)
			      {
				   ...
				   strcat(file_or_folder_to_process___, final_file_or_folder_ar);
...
char tar_filename__ar[1024 + 1       ];
char path_with_the_files__ar[1024 + 1];
 ...
 */
int
getpor_10000(int max, int fatia)
{
  double maxa;
  double fatiaa;
  maxa = (double)max;
          ...
          if (enable_encryption_arp_ && Z_NEW_MODE == internal_encryption_z_method)
            {
              val_ar = getpor_10000_int64_t_ar((int64_t)((double)((double)amanda_pereira_total_size *
								  (double)3.33333)), ricrdo_bytes_read);
            }
          else
            {
              val_ar = getpor_10000_int64_t_ar(amanda_pereira_total_size * 2, ricrdo_bytes_read);
 ...
 * don´t know assembly just ignore this information...
 *
 */
...
/**
 * This is the function that will create an iso file (internal use)
 *
 * @param tar_filename_ar the iso file to be created
 *
 * @param path_with_the_files_ar the path with the files
 * to read and scan from
 *
 * @param patern_ar the wildcard or file selection as an example
 * the information "*.ini *.txt *makefile*" will include all the
 * ini files txt files and all files that have makefile on his
 * name and will ignore the rest
 *
 * @return 0 if no error, other value otherwise
 *
 */
...
#define BUF_SIZE_ARP sizeof(our_map_arp)
  ...
  char szName_arp[1024] = "amanda_e_ricardo_map";
  ...
  global_ptr_our_map_arp = (our_map_arp *)MapViewOfFile(hMapFile_arp,          // handle to map object
                                                        FILE_MAP_ALL_ACCESS, // read/write permission
                                                        0,
                                                        0,
                                                        BUF_SIZE_ARP);

  if (NULL == global_ptr_our_map_arp)
    {
  ...
  strncpy_z(global_our_map_arp.global_patern_ar, global_patern_ar, 1022);
  global_our_map_arp.recurse_on_subfolders_arp = recurse_on_subfolders_arp;
  global_our_map_arp.mode_is_include_or_exclude__ = mode_is_include_or_exclude__;
  CopyMemory((PVOID)global_ptr_our_map_arp, &global_our_map_arp, BUF_SIZE_ARP);
  ...
  switch (iso_mode_value)
    ...
    case ARP_MODE_ISO9660:
      strcpy(my_command_call_arp, "be_mkisofs_cygwin64.exe -o \"");
      break;

    case ARP_MODE_JOLIET:
      strcpy(my_command_call_arp, "be_mkisofs_cygwin64.exe -J -o \"");
      break;

    case ARP_MODE_ROCK_RIDGE:
      strcpy(my_command_call_arp, "be_mkisofs_cygwin64.exe -R -o \"");
      break;

    case ARP_MODE_JOLIET_PLUS_ROCK_RIDGE:
      strcpy(my_command_call_arp, "be_mkisofs_cygwin64.exe -R -J -o \"");
      break;

    case ARP_MODE_UDF:
      strcpy(my_command_call_arp, "be_mkisofs_cygwin64.exe -UDF -o \"");
      break;
  ...
  strcat(command_line_arp, tar_filename_ar);
  strcat(command_line_arp, "\" \"");
  strcat(command_line_arp, path_with_the_files_ar);
  strcat(command_line_arp, "\" ");
  ...
  if (!CreateProcess(NULL, command_line_arp, NULL, NULL, FALSE, 0, NULL, NULL, &si, &pi))
    {
      sprintf(copy_arp, "Could not start executable 'be_mkisofs_cygwin64.exe' (%d)",
              (int)GetLastError());
      strcpy(error_message_k, copy_arp);
      UnmapViewOfFile(global_ptr_our_map_arp);
      CloseHandle(hMapFile_arp);
      global_ptr_our_map_arp = NULL;
      return 28004;
    }
  ...
  if (exit_code_arp)
    {
      ...
      sprintf(copy_arp, "Error %d running 'be_mkisofs_cygwin64.exe'", (int)exit_code_arp);
      strcpy(error_message_k, copy_arp);
      return 28005;
    }
final_arp:;
 ...
 *
 * @param patern_ar this is the iformation that will select what files
...
int __stdcall create_archive_internal_ar(char *tar_filename_ar, char * path_with_the_files_ar, char * patern_ar)
{
     ...
     char      original_destination_tar_file[1024] = {0};
     char      copy_ar[1024];
     DWORD     attributes;
     char      exit_data_ar[1024];
      ...
      if (0 == fatal_exit_k)
        {
          ...
          switch (returnvalue_ar)
            ...
            default:
		      ...
		      "not handled correctly by the programmer", returnvalue_ar);
          ...
          if ((1 < strlen(temp___files_ar)) && (1 < strlen(temp_folders_ar)))
            {
              snprintf(error_message_k, 600, "At least%sand%swas skipped because the path "
		       "was larger than MAX_PATH (260)", temp___files_ar, temp_folders_ar);
            }
          else
            {
