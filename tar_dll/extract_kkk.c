 ...
 */
int
stripfilenameandpath_unix_arp(char *path, char *onlypath, char *onlyfilename)
{
  char*ptr;
...
int __stdcall
stripfilenameandpath_amanda_s_smart_ape(char *path, char *onlypath, char *onlyfilename)
{
	return
          ...
          if (temp[i] == '\\')
            {
              ...
              if (0 == ispathfolder(temp))
                CreateDirectoryW(amanda_utf8towide_1_(temp), ((void *)0));
                 ...
                 __attribute__((unused)) mode_t *current_mode_mask, char *constructed_filename_kp)
{
  ...
  int openflag = (O_WRONLY | O_BINARY
                  | O_CREAT
                  | O_TRUNC);      
                     ...
                     __attribute__((unused)) mode_t *current_mode_mask, __attribute__((unused)) char *constructed_filename_kp)
{
...

  {
    ...
    if (!SetFileAttributesW(
          amanda_utf8towide_1_(destination_file_kp),
  ...
  if (!mode_is_update_arp)
    {
      ...
      if (!SetFileAttributesW(
            amanda_utf8towide_1_(constructed_filename_kp),
