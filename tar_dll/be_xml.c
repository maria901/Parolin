 ...
 *
 * @param root__________feline the root, if it donï¿½t exist it will be created
 *
 * @param node__________feline the node, if it donï¿½t exist it will be created
 *
		...
		if (-1 != valid_position_feline)
		{
			...
			if (my_file_feline)
			{
				...
				while (fgets(line_buf_feline, xml_file_size_feline, my_file_feline))
				{
					...
					if (strstr(line_buf_feline, adjusted_root_feline_b))
					{
						internal_mode_feline = FELINE_MODE_INCLUDE_; //habilita novamente a adiï¿½ï¿½o
					}
				}
