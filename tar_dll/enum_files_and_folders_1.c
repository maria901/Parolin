...
loop_again_amanda_s_smart_ape:;
	...
	if (INVALID_HANDLE_VALUE == oi_amanda_ptr->current_hFind_amanda_s_smart_____ape)
	{
		...
		{
			pedro_dprintf(-1, "free for�ado final on %x\n", oi_amanda_ptr);
			free(oi_amanda_ptr);
			oi_amanda_ptr = NULL;
			goto no_entries_i; //se � um subdiretorio tem . e .. experimente
		}
...
keep_running_amanda_s_smart_ape:;   //one less bug..
	...
	if (oi_amanda_ptr_inicial != oi_amanda_ptr)
	{
		...
		pedro_dprintf(IS_DEBUG_APE__, "nao � tem que processar\n");
		//vamos l�...debugar isto bem amor... /* here for historical reasons
