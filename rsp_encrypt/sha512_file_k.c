...
{
	SHA512_CTX *c = (SHA512_CTX *)ctx;
	assert(c);
	SHA512_Update(c, buffer_k, len_k);
	return;
}
...
void sha512_final_k(void *ctx, unsigned char *array_digest_64_bytes)
{
	SHA512_CTX *c = (SHA512_CTX *)ctx;
	assert(c);
	SHA512_Final(array_digest_64_bytes, c);
	free(ctx);
}
/**
 * buffer_k       - the input buffer t retrieve the SHA-512
 *    len_k       - the size of the buffer
 * array_digest_k - the 64 bytes to hold the digest
 */
void get_sha_512_as_an_array_of_bytes_k(unsigned char *buffer_k, unsigned int len_k, unsigned char *array_digest_k)
{
	memcpy(array_digest_k, SHA512(buffer_k, len_k, NULL), SHA512_DIGEST_LENGTH);
	return;
}
 ...
 * buffer_k - the input buffer t retrieve the SHA-512
 *    len_k - the size of the buffer
 * digest_k - the 129 bytes array of unsigned char for the output digest, it
 *            is 128 for the digest plus 1 for the end of string character
 */
void get_sha_512_as_string_k(unsigned char *buffer_k, unsigned int len_k, unsigned char *digest_k)
{
	int i_k = 0;
	unsigned char *ola_k = digest_k;
	memset(digest_k, 0, (SHA512_DIGEST_LENGTH * 2) + 1);
	unsigned char temp_out_k[SHA512_DIGEST_LENGTH] = {0};
	memcpy(temp_out_k, SHA512(buffer_k, len_k, NULL), SHA512_DIGEST_LENGTH);
	for (i_k = 0; i_k < SHA512_DIGEST_LENGTH; i_k++)
	{
		sprintf((char *)ola_k, "%02x", temp_out_k[i_k]);
		ola_k += 2;
	}
}
 ...
 *
 */
int SHA512_filelong_k(unsigned char *inputfile, unsigned char *sha512_digest_k)
{

	static unsigned char buf[AMANDA_TAMANHO];
	HANDLE stream;
	unsigned int len;
	...
	ret_z_1:
		...
		sha512_update_k(ptr_sha512_k, buf, len);
...
{
	int i_k = 0;
	unsigned char temp_k[1000] = {0};
	unsigned char temp_out_k[1000] = {0};
	unsigned char temp_out2_k[1000] = {0};

	char *temp_2_k;

	unsigned char *ola_k = temp_out2_k;

	strcpy((char *)temp_k, data_arp);

	temp_2_k = sha512_init_k();

	sha512_update_k((void *)temp_2_k, temp_k, strlen((char *)temp_k));

	sha512_final_k((void *)temp_2_k, temp_out_k);
	...
	strcpy(out_arp, (char *)temp_out2_k);
...
int main()
{
	...
	unsigned char temp_k[1000] ={0};
	unsigned char temp_out_k[1000] ={0};
	unsigned char temp_out2_k[1000] = "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa";

	char * temp_2_k;

	unsigned char * ola_k = temp_out2_k;

	strcpy(temp_k, "ricardo");

	temp_2_k = sha512_init_k();

	sha512_update_k((void *) temp_2_k, temp_k, strlen(temp_k));
	sha512_update_k((void *) temp_2_k, temp_k, strlen(temp_k));
	...
	for(i_k = 0; i_k < SHA512_DIGEST_LENGTH; i_k++)
	{
		sprintf(ola_k, "%02x", temp_out_k[i_k]);
		ola_k += 2;
	}

	printf(" saiu %s\n SHA512_DIGEST_LENGTH == %d", temp_out2_k, SHA512_DIGEST_LENGTH);
}
#endif