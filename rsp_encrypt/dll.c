...
int __stdcall DllMain(__attribute__((unused)) HANDLE hModule, DWORD ul_reason_for_call, __attribute__((unused)) LPVOID lpReserved)
{
     ...
     case DLL_PROCESS_ATTACH:
          OutputDebugString("AR Encrypt DLL DLL_PROCESS_ATTACH (" __TIME__ " " __DATE__ ") \n");

#ifdef NPRINTF
          //Beep (2000, 50);
          MessageBox(0, "cardo debugging code...", "BW", MB_OK | MB_TOPMOST);
#endif
...
int __stdcall newfileencrypt2_sha512(uint type, uchar *inputfile, uchar *outputfile, uchar *key)
{
     ...
     unsigned char sha512_digest_b_k[65] = {
         0,
     }; //for safety
     ...
     if ((stream = _wfopen(permissive_name_m_(amanda_utf8towide_1_((char *)inputfile)), L"rb")) != NULL)
     {
     }
     else
     {
     ...
     if ((stream2 = _wfopen(permissive_name_m_(amanda_utf8towide_1_((char *)outputfile)), L"wb")) != NULL)
     {
     }
     else
     {
...
int encrypt_arp(uchar *inputfile,
                uchar *outputfile,
                uchar *key,
                int encryption_method_internal_arp,
                int number_of_threads_ar)
{
     int ret_arp;
     char key_arp[300] = {0};
     (void)number_of_threads_ar;
