...
int
aes_set_key (aes_context * ctx, uint8 * key, int nbits)
{
	int i;
	uint32 *RK, *SK;
	switch (nbits)
...
void
aes_encrypt (aes_context * ctx, uint8 input[16], uint8 output[16])
{
...
void
aes_decrypt (aes_context * ctx, uint8 input[16], uint8 output[16])
{
	uint32 *RK, X0, X1, X2, X3, Y0, Y1, Y2, Y3;
	RK = ctx->drk;
	GET_UINT32 (X0, input, 0);
...
int
aesencrypt (unsigned char *buf, unsigned char *bufout, unsigned char *key, int keysize, int *done)
{

	static aes_context ctx;
	static aes_context ctx2;
	...
	aes_encrypt (&ctx, buf, bufout);
	return 0;
}

int
aesdecrypt (unsigned char *buf, unsigned char *bufout, unsigned char *key, int keysize, int *done)
{
	...
	{
#ifdef NPRINTF
		dprintf ("aes dec primeiro debug \n");
#endif
		aes_set_key (&ctx, key, keysize);
...
main (void)
{
	...
	for (m = 0; m < 2; m++)
	{
		...
		for (n = 0; n < 3; n++)
		{
			...
			if ((m == 0 && memcmp (buf, AES_enc_test[n], 16)) || (m == 1 && memcmp (buf, AES_dec_test[n], 16)))
			{
#ifdef NPRINTF
				printf ("failed!\n");
#endif
