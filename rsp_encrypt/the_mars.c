...
#define f_ktr(a, b, c, d, i)                \
     m = a + ptr_my_struct_z->l_key[i];     \
     a = rotl(a, 13);                       \
     r = a * ptr_my_struct_z->l_key[i + 1]; \
     l = s_box[m & 511];                    \
     r = rotl(r, 5);                        \
     c += rotl(m, r);                       \
     l ^= r;                                \
...
static u4byte *set_key_v27(const u4byte in_key[], const u4byte key_len, my_thread_struct_z *ptr_my_struct_z)

{
     u4byte i, j, m, w;
     int i_m;

     for (i_m = 0; i_m < 47; i_m++)
     {
          ptr_my_struct_z->vk[i_m] = vk_ori[i_m];
     }
     ...
     for (i = j = 0; i < 39; ++i)
     {
          ...
          ptr_my_struct_z->vk[i + 7] = rotl(ptr_my_struct_z->vk[i] ^ ptr_my_struct_z->vk[i + 5], 3) ^ in_key[j] ^ i;

          j = (j == m ? 0 : j + 1);
     }

     ptr_my_struct_z->vk[46] = key_len / 32;
...

     {
          ...
          for (i = 1; i < 40; ++i)
               ...
               ptr_my_struct_z->vk[i + 7] = rotl(ptr_my_struct_z->vk[i + 7] + s_box[ptr_my_struct_z->vk[i + 6] & 511], 9);

          ptr_my_struct_z->vk[7] = rotl(ptr_my_struct_z->vk[7] + s_box[ptr_my_struct_z->vk[46] & 511], 9);
     }
...

     {
          ...
          ptr_my_struct_z->l_key[j] = ptr_my_struct_z->vk[i + 7];
...

     {
          ...
          w = ptr_my_struct_z->l_key[i] | 3;

          if ((m = gen_mask(w)))

               w ^= (rotl(s_box[265 + (ptr_my_struct_z->l_key[i] & 3)], ptr_my_struct_z->l_key[i + 3] & 31) & m);

          ptr_my_struct_z->l_key[i] = w;
     }

     return ptr_my_struct_z->l_key;
};
...
static void encrypt_v27(const u4byte in_blk[4], u4byte out_blk[4], my_thread_struct_z *ptr_my_struct_z)
{
     u4byte a, b, c, d, l, m, r;

     a = in_blk[0] + ptr_my_struct_z->l_key[0];
     b = in_blk[1] + ptr_my_struct_z->l_key[1];

     c = in_blk[2] + ptr_my_struct_z->l_key[2];
     d = in_blk[3] + ptr_my_struct_z->l_key[3];
     ...
     out_blk[0] = a - ptr_my_struct_z->l_key[36];
     out_blk[1] = b - ptr_my_struct_z->l_key[37];

     out_blk[2] = c - ptr_my_struct_z->l_key[38];
     out_blk[3] = d - ptr_my_struct_z->l_key[39];
};

static void decrypt_v27(const u4byte in_blk[4], u4byte out_blk[4], my_thread_struct_z *ptr_my_struct_z)
{
     u4byte a, b, c, d, l, m, r;

     d = in_blk[0] + ptr_my_struct_z->l_key[36];
     c = in_blk[1] + ptr_my_struct_z->l_key[37];

     b = in_blk[2] + ptr_my_struct_z->l_key[38];
     a = in_blk[3] + ptr_my_struct_z->l_key[39];
     ...
     out_blk[0] = d - ptr_my_struct_z->l_key[0];
     out_blk[1] = c - ptr_my_struct_z->l_key[1];

     out_blk[2] = b - ptr_my_struct_z->l_key[2];
     out_blk[3] = a - ptr_my_struct_z->l_key[3];
}