     ...
     uInt dictLength;
{
   ...
   rspmemcpy (s->window, dictionary, length);
   s->strstart = length;
     ...
     z_streamp strm;
{
   ...
   rspmemcpy (strm->next_out, strm->state->pending_out, len);
   strm->next_out += len;
     ...
     z_streamp source;
{
   ...
   *ds = *ss;
   ...
   /* following rspmemcpy do not work for 16-bit MSDOS */

//Sleep(1);
   rspmemcpy (ds->window, ss->window, ds->w_size * 2 * sizeof (Byte));
   rspmemcpy (ds->prev, ss->prev, ds->w_size * sizeof (Pos));
   rspmemcpy (ds->head, ss->head, ds->hash_size * sizeof (Pos));
   rspmemcpy (ds->pending_buf, ss->pending_buf, (uInt) ds->pending_buf_size);
     ...
     unsigned size;
{
   ...
   rspmemcpy (buf, strm->next_in, len);
   strm->next_in += len;
   strm->total_in += len;
     ...
     deflate_state *s;
{
   ...
   do
	...
	if (more == 0 && s->strstart == 0 && s->lookahead == 0)
	  ...
	  {
//Sleep(1);
	     rspmemcpy (s->window, s->window + wsize, (unsigned) wsize);
	     s->match_start -= wsize;
