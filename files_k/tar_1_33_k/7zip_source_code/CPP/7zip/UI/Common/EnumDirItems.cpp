...
extern int64_t recurse_on_subfolders__i;
  ...
  */
  ...
  FOR_VECTOR (i, files)
  {
    ...
    */
	...
	if(fi.IsDir())
	{
		if(0 == recurse_on_subfolders__i)//don`t recurse
		return S_OK;
	...
	{
		//return S_OK;
		NFind::CFileInfo fi_pedro = fi;
		if(match_i(valquiria_wide_to_utf8(fi_pedro.Name.GetBuf())))
		{
			RINOK(EnumerateForItem(fi, curNode, phyParent, logParent, phyPrefix,
			addParts, dirItems, enterToSubFolders));
			if (dirItems.Callback && (i & kScanProgressStepMask) == kScanProgressStepMask)
