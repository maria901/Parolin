...
extern int seven_zip_errors_i;

int is_encrypted_i;

char zip_description_i[1024];
...
int        Day_k_ar_;
__int64    Size_k_ar_;
char       filename_k_ar_[1027];
int        Numdir_k_ar_;
int        Numfiles_k_ar_;
char       typeflag_ar__[1027];
  ...
  */
  ...
  FOR_VECTOR (i, _fields)
  {
    ...
    if (f.PropID == kpidPath)
    {
	  ...
	  //28 may 2021 //OutputDebugStringW(FilePath.GetBuf());
	  //28 may 2021 //OutputDebugStringW(TempWString.GetBuf());
	  //28 may 2021 //OutputDebugStringA(TempAString.GetBuf());

		strcpy(filename_k_ar_, valquiria_wide_to_utf8(FilePath.GetBuf()));

		set_info_libarchive_i
		(
			Isdir_k_ar_,
			Second_k_ar_,
			Minute_k_ar_,
    ...
    else
    {
      ...
      if (f.PropID == kpidAttrib && (prop.vt == VT_EMPTY || prop.vt == VT_UI4))
      {
		...
		if(IsDir)
		{
			Isdir_k_ar_ = 1;
			Numdir_k_ar_++;
			strcpy(typeflag_ar__, "Directory");
		}
		else
		{
			Isdir_k_ar_ = 0;
			Numfiles_k_ar_++;
			strcpy(typeflag_ar__, "File");
			
			if(is_encrypted_i)
			{
				
				strcat(typeflag_ar__, " (Encrypted)");
		...
		{
			printf(" oi2 ->%s <- ->%s-<", temp, temp + tempPos);
      ...
      else if (prop.vt == VT_FILETIME)
      {
		...
		
		{
			...
			FileTimeToSystemTime(&prop.filetime, &stUTC);
			SystemTimeToTzSpecificLocalTime(NULL, &stUTC, &stLocal);

			Year_k_ar_ = stLocal.wYear;
			Month_k_ar_ = stLocal.wMonth;
			Day_k_ar_ = stLocal.wDay;

			Hour_k_ar_ = stLocal.wHour;
			Minute_k_ar_ = stLocal.wMinute;
			Second_k_ar_ = stLocal.wSecond;			
...
static void PrintPropertyPair2(CStdOutStream &so, PROPID propID, const wchar_t *name, const CPropVariant &prop)
{
  ...
  if (!s.IsEmpty())
  {
	...
	if(22 == propID)
	{
		...
		{
			char ricardo_[1027];
			strcpy(ricardo_, "property id %d\n");
			pedro_dprintf(-1, ricardo_, propID);
		}
		*/
		
		//28 may 2021 //OutputDebugStringW(L"Ahhhhh....");
		//28 may 2021 //OutputDebugStringW(s.GetBuf());
		
		strcpy(zip_description_i, valquiria_wide_to_utf8(s.GetBuf()));
		
		if(strstr( valquiria_wide_to_utf8(s.GetBuf()) , "7zAES"))
		{
			is_encrypted_i = 1;
			...
			{
				char ricardo_[1027];
				strcpy(ricardo_, "Is Encrypted\n");
				pedro_dprintf(-1, ricardo_);
			}
			*/
		}
		else
...
static HRESULT PrintArcProp(CStdOutStream &so, IInArchive *archive, PROPID propID, const wchar_t *name)
{
	...
	//28 may 2021 OutputDebugStringW(L"Agora amor...");
	
	if(NULL == name)
	{
		;// //28 may 2021 OutputDebugStringW(L"NULL querido");
	}
	//28 may 2021 //OutputDebugStringW(name);
    ...
    UInt64 &numWarnings)
{
  ...
  for (arcIndex = 0; arcIndex < arcPaths.Size(); arcIndex++)
  {
    ...
    */
	...
	UString copia_w = arcPath;
	//28 may 2021 OutputDebugStringW(copia_w.GetBuf());
  ...
  {
	char ricardo_[1027];
	strcpy(ricardo_, "Numitens %d\n");
	pedro_dprintf(-1, ricardo_, numItems);
  }
	*/
