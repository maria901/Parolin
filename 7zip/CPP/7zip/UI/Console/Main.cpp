...
char ricardo_[1027] = "oi";

extern "C" {
	
static WCHAR * amanda_utf8towide_1_(char *pUTF8)
{
	static WCHAR ricardo_k[1027];
...
*/
	...
	*/
	  const UString s2(amanda_utf8towide_1_(args[i]));
	  
      commandStrings.Add(s2);
    }
  ...
  */
  ...
  else if (isExtractGroupCommand || options.Command.CommandType == NCommandType::kList)
  {
    ...
    if (isExtractGroupCommand)
    {
      ...
      */
	  ...
	  {
			char ricardo_[1027];
			strcpy(ricardo_, "Is Extract my love...\n");
			pedro_dprintf(-1, ricardo_);
		}
	  */
  ...
  {
	char ricardo_[1027];
	strcpy(ricardo_, "List archives amor?\n");
	pedro_dprintf(-1, ricardo_);
  }
	*/
	
  printf("Oi meu querido?...\n");
  ...
  else if (options.Command.IsFromUpdateGroup())
  {
	...
	//28 may 2021 //OutputDebugStringW(vamos_la.GetBuf());
  ...
  hInstance, DWORD dwReason, LPVOID /*lpReserved*/)
{
  ...
  {
    /*Ricardo nao comente... */ OutputDebugStringA("BE 7zip DLL - DLL_PROCESS_ATTACH (" __TIME__ " " __DATE__ ") _amanda_debug_");
  }
