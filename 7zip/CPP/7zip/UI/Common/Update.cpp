    ...
    CFinishArchiveStat &st)
{
  ...
  if (options.RenamePairs.Size() != 0)
  {
    FOR_VECTOR (i, arcItems)
    {
	  ...
	  //28 may 2021 //OutputDebugStringW(ai.Name);
  ...
  else
  {
    ...
    //28 may 2021 OutputDebugStringW(L"aquia");
...

  {
	...
	/*
	  {
			char ric[300] =  "tam %d\n";
			pedro_dprintf(-1, ric, updatePairs2.Size());
	  }
	*/
    ...
    CObjectVector<CArcItem> &arcItems)
{
  ...
  for (UInt32 i = 0; i < numItems; i++)
  {
	...
	if(!ai.IsDir)
	{
		...
		if(false == match_i(valquiria_wide_to_utf8(ai.Name.GetBuf())))
		{
			
	{
		char ric[300] =  "Excluindo %s \n";
		pedro_dprintf(-1, ric, valquiria_wide_to_utf8(ai.Name.GetBuf()));
	}
			continue;
		}	
    ...
    bool needSetPath)
{
  ...
  */
  ...
  else
  {
	...
	//28 may 2021 //OutputDebugStringW(L"Antes _amanda_debug_");
      HRESULT res = EnumerateItems(censor,
          options.PathMode,
          UString(), // options.AddPathPrefix,
          dirItems);

	//28 may 2021 //OutputDebugStringW(L"depois _amanda_debug_");
  ...
  */
  ...
  for (ci = 0; ci < options.Commands.Size(); ci++)
  {
    ...
    RINOK(Compress(options,
		...
		//28 may 2021 OutputDebugStringW(L"Depois ok _amanda_debug_");
