...
static void default_bzfree(__attribute__((unused)) void *opaque, void *addr)
{
     if (addr != NULL)
          free(addr);
}
                                 ...
                                 int open_mode) /* bzopen: 0, bzdopen:1 */
{
     ...
     if (open_mode == 0)
     {
          ...
          else
          {
               ...
               pedro_dprintf(-1, "abrindo arquivo %s", path);
               if (unicodemode)
               {
                    pedro_dprintf(-1, "Modo unicode");

                    {
                         short temp_w2_k[300] = {0};
                         WCHAR *ar_temp = (void *)malloc(AMANDA__SIZE_ww);
                         WCHAR *ar_temp2 = (void *)malloc(AMANDA__SIZE_ww);
                         wcscpy((void *)temp_w2_k, amanda_utf8towide_1_v28(mode2, ar_temp));
                         fp = _wfopen(permissive_name_m_v28(amanda_utf8towide_1_v28((void *)path, ar_temp), ar_temp2), (unsigned short *)temp_w2_k);
                    ...
                    fp = fopen(path, mode2), pedro_dprintf(-1, "Modo ANSI");
          }
     }
     else
     {
