...
static void safe_fwrite(void* buf, size_t eltSize, size_t nbElt, FILE_z* f, my_thread_struct_z * amanda)
{
	size_t const writtenSize = fwrite_z(buf, eltSize, nbElt, f);
	...
	amanda->size_of_destination_file_z += writtenSize;
	
}

static void safe_fwrite_dec(void* buf, size_t eltSize, size_t nbElt, FILE* f, my_thread_struct_z * amanda)
{
	if(true == amanda->dest_is_FILE_z)
	{
		assert(amanda->dest);
		size_t const writtenSize = fwrite_z(buf, eltSize, nbElt, amanda->dest);
		size_t const expectedSize = eltSize * nbElt;
		//if (nbElt>0) assert(expectedSize / nbElt == eltSize);  /* check overflow */
		if (writtenSize < expectedSize)
		{
...
compress_file_internal(FILE* f_in, FILE_z* f_out,
                       LZ4F_compressionContext_t ctx,
                       void* inBuff,  size_t inChunkSize,
                       void* outBuff, size_t outCapacity, my_thread_struct_z * amanda)
{
	...
	/* write frame header */
	{
		...
		safe_fwrite(outBuff, 1, headerSize, f_out, amanda);

		if(amanda->internal_error_arp)
		{
			return result;
		}
	}
	...
	for (;;)
	{
		size_t const readSize = fread(inBuff, 1, min(IN_CHUNK_SIZE, amanda->size_of_input_file_copy_z), f_in);
		...
		count_in += readSize;
		amanda->size_of_input_file_copy_z -= readSize;
		assert(0 <= amanda->size_of_input_file_copy_z);
		*((*amanda).bytes_read_z)            +=  readSize;

		pedro_dprintf(-1, "dentro %lld\n", *((*amanda).bytes_read_z));
		...
		safe_fwrite(outBuff, 1, compressedSize, f_out, amanda);
...
decompress_file_internal(FILE* f_in, FILE* f_out,
                         ...
                         void* dst, size_t dstCapacity, my_thread_struct_z * amanda)
{
	int firstChunk = 1;
	size_t ret = 1;
	...
	while (ret != 0)
	{
		...
		{
			readSize = fread(src, 1, min(srcCapacity, amanda->size_of_input_file_copy_z), f_in);
			amanda->size_of_input_file_copy_z -= readSize;
			*((*amanda).bytes_read_z)         += readSize;
		}
		firstChunk = 0;
		...
		{
			pedro_dprintf(-1, "Decompress: not enough input or error reading file\n");
			return 1;
		}
		pedro_dprintf(-1, "4\n");
		/* Decompress:
		...
		while (srcPtr < srcEnd && ret != 0)
		{
			...
			if (LZ4F_isError(ret))
			{
				pedro_dprintf(-1, "Decompression error: %s\n", LZ4F_getErrorName(ret));
				return 1;
			}
			/* Flush output */
			if (dstSize != 0) safe_fwrite_dec(dst, 1, dstSize, f_out, amanda);

			if(amanda->internal_error_arp)
			{
				pedro_dprintf(-1, "Decompression error 5: \n");
				return 1;
			}
		...
		if (srcPtr < srcEnd)
		{
			pedro_dprintf(-1, "Decompress: Trailing data left in file after frame\n");
			return 1;
		}
	}
...
decompress_file_allocDst(FILE* f_in, FILE* f_out,
                         LZ4F_dctx* dctx,
                         void* src, size_t srcCapacity, my_thread_struct_z * amanda)
{
	assert(f_in != NULL )     ;
	...
	pedro_dprintf(-1, "decompress_file_allocDst 1 size %lld\n", amanda->size_of_input_file_copy_z);

	/* Read Frame header */
	size_t const readSize = fread(src, 1, min(srcCapacity, amanda->size_of_input_file_copy_z), f_in);
	if (readSize == 0 || ferror(f_in))
	{
		pedro_dprintf(-1, "Decompress: not enough input or error reading file\n");
		return 1;
...
int decompress_file(FILE* f_in, FILE* f_out, my_thread_struct_z * amanda)
{
	...
	int const result = !dctx ? 1 /* error */ :
	                   decompress_file_allocDst(f_in, f_out, dctx, src, IN_CHUNK_SIZE, amanda);
