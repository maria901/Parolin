...
extern WCHAR *__stdcall utf8towide_no(const char *pUTF8);
                                 ...
                                 __attribute__((unused)) my_thread_struct_z *amanda)
{
	...
	*f = _wfopen(utf8towide(input_path), L"rb");
	if (!*f)
	{
...
static int64_t FileSize(const char *path)
{
     FILE *f = NULL;
     {
          ...
          f = _wfopen(permissive_name_m_v28(amanda_utf8towide_1_v28((void *)path, ar_temp), ar_temp2), L"rb");
...
static void CopyStat(const char *input_path, const char *output_path)
{
     ...
     times.modtime = statbuf.st_mtime;
     {
          ...
          _wutime(permissive_name_m_v28(amanda_utf8towide_1_v28((void *)output_path, ar_temp), ar_temp2), &times);
...
static BROTLI_BOOL NextFile(Context *context, my_thread_struct_z *amanda)
{
     const char *arg;
     size_t arg_len;
     ...
     context->input_file_length = amanda->size_of_input_file_z; //FileSize(arg);//aqui tem que colocar o tamanho do arquivo...
     context->current_output_path = context->output_path;
...
#ifdef ARP_DEBUG__
     tamanho____aakp = getfilesize_aakp_plus_rspk((char *)context->current_input_path);
#endif
...
static BROTLI_BOOL CloseFiles(Context *context, BROTLI_BOOL success)
{
     ...
     if (!context->test_integrity && context->fout)
     {
          if (!success && context->current_output_path)
          {
               ...
               _wunlink(permissive_name_m_v28(amanda_utf8towide_1_v28((void *)(context->current_output_path), ar_temp), ar_temp2));
     ...
     if (success && context->junk_source && context->current_input_path)
     {
          ...
          _wunlink(permissive_name_m_v28(amanda_utf8towide_1_v28((void *)(context->current_input_path), ar_temp), ar_temp2));
...
static BROTLI_BOOL WriteOutput(Context *context, my_thread_struct_z *amanda)
{
     size_t ret_val;
     ...
     if (Z_MODE_IS_COMPRESSION == amanda->our_mode_z)
     {
          ret_val = fwrite_z(context->output, 1, out_size, amanda->dest);
     }
     else if (Z_MODE_IN_DECOMPRESSION == amanda->our_mode_z)
     {
...
static BROTLI_BOOL ProvideOutput(Context *context, my_thread_struct_z *amanda)
{
     if (!WriteOutput(context, amanda))
          return BROTLI_FALSE;
     context->available_out = kFileBufferSize;
     context->next_out = context->output;
     return BROTLI_TRUE;
...
static BROTLI_BOOL FlushOutput(Context *context, my_thread_struct_z *amanda)
{
     if (!WriteOutput(context, amanda))
          return BROTLI_FALSE;
...
static BROTLI_BOOL DecompressFile(Context *context, BrotliDecoderState *s, my_thread_struct_z *amanda)
{
     ...
     for (;;)
     {
          ...
          {
               pedro_dprintf(-1, "BROTLI_DECODER_RESULT_NEEDS_MORE_INPUT\n");
               ...
               if (!ProvideInput(context, amanda))
                    return BROTLI_FALSE;
               pedro_dprintf(-1, "passou 2\n");
          }
          else if (result == BROTLI_DECODER_RESULT_NEEDS_MORE_OUTPUT)
          {
               pedro_dprintf(-1, "BROTLI_DECODER_RESULT_NEEDS_MORE_OUTPUT\n");
               if (!ProvideOutput(context, amanda))
                    return BROTLI_FALSE;
          }
          else if (result == BROTLI_DECODER_RESULT_SUCCESS)
          {
               pedro_dprintf(-1, "BROTLI_DECODER_RESULT_SUCCESS\n");
               if (!FlushOutput(context, amanda))
                    return BROTLI_FALSE;
               int has_more_input =
          ...
          {
               fprintf(stderr, "corrupt input 3 [%s]\n",
                       PrintablePath(context->current_input_path));
               return BROTLI_FALSE;
          }
...
static BROTLI_BOOL DecompressFiles(Context *context, my_thread_struct_z *amanda)
{
     while (NextFile(context, amanda))
     {
          BROTLI_BOOL is_ok = BROTLI_TRUE;
          ...
          is_ok = OpenFiles(context, amanda);
...
static BROTLI_BOOL CompressFile(Context *context, BrotliEncoderState *s, my_thread_struct_z *amanda)
{
     ...
     for (;;)
     {
          if (context->available_in == 0 && !is_eof)
          {
               if (!ProvideInput(context, amanda))
                    return BROTLI_FALSE;
               is_eof = !HasMoreInput(context, amanda);
          }
...
static BROTLI_BOOL CompressFiles(Context *context, my_thread_struct_z *amanda)
{
     while (NextFile(context, amanda))
     {
          BROTLI_BOOL is_ok = BROTLI_TRUE;
          ...
          {
               pedro_dprintf(-1, "Use -h help. Use -f to force output to a terminal.\n");
               is_ok = BROTLI_FALSE;
          }
          if (is_ok)
          {
...
int main_z(int argc, char **argv, my_thread_struct_z *amanda)
{
     ...
     case COMMAND_COMPRESS:
          is_ok = CompressFiles(&context, amanda);
          break;

     case COMMAND_DECOMPRESS:
     case COMMAND_TEST_INTEGRITY:
          is_ok = DecompressFiles(&context, amanda);
          break;
...
int decompress_file(FILE *f_in, FILE *f_out, my_thread_struct_z *amanda)
{
     ...
     my_main_args[1] = "-d";
     my_main_args[2] = "-f";
     my_main_args[3] = "input_file.txt.z";
     my_main_args[4] = "-o";
     my_main_args[5] = "output_file.txt.z";
