/* Copyright 2014 Google Inc. All Rights Reserved. modified by the 'guy' in 2021
...
static BROTLI_BOOL OpenInputFile(const char *input_path, FILE **f)
{
      ...
      *f = _wfopen(amanda_utf8towide_1_v27(input_path), L"rb");
      if (!*f)
      {
                                  ...
                                  BROTLI_BOOL force)
{
      ...
      *f = NULL;
      ...
      fd = _wopen(amanda_utf8towide_1_v27(output_path), O_BINARY | O_CREAT | (force ? 0 : O_EXCL) | O_WRONLY | O_TRUNC,
                  S_IRUSR | S_IWUSR);
      if (fd < 0)
      {
...
{
      FILE *f = _wfopen(amanda_utf8towide_1_v27(path), L"rb");
      int64_t retval;
      if (f == NULL)
      {
            return -1;
...
static void CopyStat(const char *input_path, const char *output_path)
{
      ...
      _wutime(amanda_utf8towide_1_v27(output_path), &times);
...
static BROTLI_BOOL CloseFiles(Context *context, BROTLI_BOOL success)
{
      ...
      if (!context->test_integrity && context->fout)
      {
            ...
            {
                  _wunlink(amanda_utf8towide_1_v27(context->current_output_path));
            }
      ...
      {
            _wunlink(amanda_utf8towide_1_v27(context->current_input_path));
      }
...
static const size_t kFileBufferSize = 4096; //1 << 19;
