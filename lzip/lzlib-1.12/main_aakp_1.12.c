/*
	...
	About updates: need to set to auto the static variables of main

	need to comment and update information about output_filename

	need to update encoder_base.h

CoffGuy, march/2021
*/

/* Minilzip - Test program for the library lzlib, slightly modified by MathMan from BinaryWork
...
static int get_dict_size(const char *const arg)
{
      ...
      if (dictionary_size == 65535)
            ++dictionary_size; /* no fast encoder */
                         ...
                         const bool one_to_one, const bool recompress)
{
      ...
      {
            infd = _wopen(amanda_utf8towide_1_v27((char *)name), O_RDONLY | O_BINARY);
            if (infd < 0)
...
static bool open_outstream(const bool force, const bool protect)
{
      ...
      outfd = _wopen(amanda_utf8towide_1_v27(output_filename), flags, outfd_mode);
      if (outfd >= 0)
...
static bool check_tty_out(const enum Mode program_mode)
{
      ...
      {
            show_file_error(output_filename[0] ? output_filename : "(stdout)",
...
static void close_and_set_permissions(const struct stat *const in_statsp)
{
      ...
      if (in_statsp)
      {
            ...
            if (_wutime(amanda_utf8towide_1_v27(output_filename), &t) != 0)
                  warning = true;
      }
                    ...
                    const struct stat *const in_statsp)
{
      struct LZ_Encoder *const encoder =
          LZ_compress_open(encoder_options->dictionary_size,
                           encoder_options->match_len_limit, (volume_size > 0) ? min(member_size, volume_size) : member_size);
      int retval;
      ...
      if (!encoder || LZ_compress_errno(encoder) != LZ_ok)
      {
            ...
            retval = do_compress(encoder, member_size, volume_size,
                                 infd, pp, in_statsp);
                         ...
                         const bool loose_trailing, const bool testing)
{
      ...
      for (first_member = true;;)
      {
            ...
            if (out_size < 0 || (first_member && out_size == 0))
            {
                  ...
                  {
                        Pp_show_msg(pp, mem_msg);
                        return 1;
                  }
                  if (verbosity >= 0)
                  {
                        Pp_show_msg(pp, 0);
                        fprintf(stderr, "%s at pos %llu\n", (lz_errno == LZ_unexpected_eof) ? "File ends unexpectedly" : "Decoder error",
                      ...
                      const bool loose_trailing, const bool testing)
{
      ...
      if (!decoder || LZ_decompress_errno(decoder) != LZ_ok)
      {
            ...
            retval = do_decompress(decoder, infd, pp, ignore_trailing,
                       ...
                       const bool help)
{
      ...
      if (msg && msg[0])
            pedro_dprintf(-1, "%s: %s%s%s\n", program_name, msg,
                          (errcode > 0) ? ": " : "",
                          (errcode > 0) ? strerror(errcode) : "");
      if (help)
            pedro_dprintf(-1, "Try '%s --help' for more information.\n",
                          invocation_name);
}
...
static void internal_error(const char *const msg)
{
      pedro_dprintf(-1, "%s: internal error: %s\n", program_name, msg);
      exit(3);
}
...
int main(const int argc, const char *const argv[])
{
      ...
      struct Arg_parser parser; /* static because valgrind complains */
      struct Pretty_print pp;   /* and memory management in C sucks */
...
#error "lzlib 1.12 or newer needed."
      ...
      if (program_mode == m_compress)
      {
            ...
            volume_size = 0;
      if (program_mode == m_test)
            to_stdout = false; /* apply overrides */
      if (program_mode == m_test || to_stdout)
            default_output_filename = "";
      pedro_dprintf(-1, "passou 1.4\n");
      //output_filename = resize_buffer( output_filename, 1 );
      output_filename[0] = 0;
      if (to_stdout && program_mode != m_test) /* check tty only once */
      {
            outfd = STDOUT_FILENO;
            if (!check_tty_out(program_mode))
                  return 1;
      }
      else
            outfd = -1;
      ...
      for (i = 0; i < num_filenames; ++i)
      {
            ...
            if (strcmp(filenames[i], "-") == 0)
            {
                  if (stdin_used)
                        ...
                        stdin_used = true;
                  infd = STDIN_FILENO;
                  if (!check_tty_in(pp.name, infd, program_mode, &retval))
                        continue;
                  if (one_to_one)
                  {
                        outfd = STDOUT_FILENO;
	...
	*/
      ...
      pedro_dprintf(-1, "passou 8\n");

      return retval;
}

int __stdcall DllMain(__attribute__((unused)) HANDLE hModule,
...
int __stdcall compress_lzip_rspk(char *inputfile_utf_8,
                                 ...
                                 int level_aakp)
{
      ...
      /*
	   if(output_filename)
	   {
	    pedro_dprintf(-1, "dando free em output_filename %p %d\n", output_filename, output_filename);
	    free(output_filename);
