...
**
** Changed be compatible with GNU/Linux's compress - David J. McKellar - January 3, 2007
**
*****************************************************************************/
#include <stdio.h>
#include <stdlib.h>
...
main(int argc, char *argv[])
{
     ...
     FILE *output_file;
     FILE *lzw_file;
     char input_file_name[256]; // djm - was 81
...
void compress(FILE *input, FILE *output)
{
     ...
     while ((character = getc(input)) != (unsigned)EOF)
     {
          ...
          {                                           /* table, try to add it.   */
               ...
               output_code(output, string_code, BITS); /* When a string is found  */
               string_code = character;                /* that is not in the table*/
                                                       ...
                                                       */
     output_code(output, string_code, BITS);           /* Output the last code               */
     /*output_code(output,MAX_VALUE);*/                /* Output the end of buffer code - not done by GNU/Linux compress - djm */
     output_code(output, 0, BITS);                     /* This code flushes the output buffer*/
     printf("\n");
}
...
//     int block_mode;
     ...
     /* read the header - djm */
     if (fgetc(input) != MAGIC_1)
          return;
     if (fgetc(input) != MAGIC_2)
          return;
     ...
     old_code = input_code(input, n_bits); /* Read in the first code, initialize the */
