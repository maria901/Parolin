...
#include "version_info_dl.h"
          ...
          volta_aqui_ric:;
               ...
               {
                    result_dl_b = mem_search_dl(buf_dl_b, DL_SIZE__, // deve funcionar, e bem
                    ...
                    else
                    {
                         if (DEBUG2_DL__)
                         {
                              pedro_dprintf(0, "found on first buffer, achou em %lld tamanho %d", result_dl_b, size_of_the_neddle_dl);
                         }
               ...
               else if (-1 != result_dl || -1 != result_dl_b)
               {
                    if (DEBUG_DL__)
...
#include "handling_current_buffer.c"
     ...
     unlink(temp_file_dl);
     printf("\nVersion of the encoder -> v8 (12 jan 2022 16:47)\n");
     return 0;
}
