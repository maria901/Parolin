    ...
    u_int32_t srclen)
{
    ...
    do
    {
        ...
        else
        {
            ...
            code_buf[code_buf_ptr++] = (u_int8_t)(((sp->match_position >> 4) & 0xF0) | (sp->match_length - (THRESHOLD + 1)));
        }
        if ((mask <<= 1) == 0)
...
int main()
{
    FILE *myfile_dl = fopen("make.exe", "rb");
...
int ret_value_dl;
    ...
    size_of_data_dl = ftell(myfile_dl);

    fseek(myfile_dl, 0, SEEK_SET);

    if (myfile_dl)
    {
        uint8_t *in__buf_dl = malloc(size_of_data_dl);
        uint8_t *out_buf_dl = malloc((size_of_data_dl * 2) + 10000);
        uint8_t *out_buf_dl2 = malloc((size_of_data_dl * 2) + 10000);

        if (fread(in__buf_dl, 1, size_of_data_dl, myfile_dl) == size_of_data_dl)
        {

            uint8_t *saida_dl;
            saida_dl = compress_lzss(out_buf_dl,
                                     (size_of_data_dl * 2) + 10000,
                                     in__buf_dl,
                                     size_of_data_dl);

            pedro_dprintf(0, "Size out %d", (int)(saida_dl - out_buf_dl));

            if (size_of_data_dl && 0 == (int)(saida_dl - out_buf_dl))
            {
                printf("Error 28\n");
                if (myfile_dl)
                    fclose(myfile_dl);
...
ret_value_dl = decompress_lzss()
        ...
        free(in__buf_dl), free(out_buf_dl), free(out_buf_dl2);
    }
