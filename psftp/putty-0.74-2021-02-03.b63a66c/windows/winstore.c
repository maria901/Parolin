 ...
 * non-null).
 */
static int transform_jumplist_registry(const char *add, const char *rem, char **out)
{
    int ret;
    HKEY pjumplist_key;
    DWORD type;
    ...
    while (((int64_t)(piterator_tmp - old_value) < (int64_t)(value_length - 1)) &&
           !(*piterator_tmp == '\0' && *(piterator_tmp + 1) == '\0'))
    {
        ++piterator_tmp;
    }

    if ((int64_t)(piterator_tmp - old_value) >= (int64_t)(value_length - 1))
    {
