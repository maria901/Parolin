...
static bool proxy_for_destination(SockAddr *addr, const char *hostname,
                                  __attribute__((unused)) int port, Conf *conf)
{
    int s = 0, e = 0;
    char hostip[64];
                         ...
                         const char *reason)
{
    return dupprintf("Looking up host \"%s\"%s for %s", host,
                     (addressfamily == ADDRTYPE_IPV4 ? " (IPv4)" : addressfamily == ADDRTYPE_IPV6 ? " (IPv6)"
                                                                                                  : ""),
                     reason);
}
    ...
    if (change == PROXY_CHANGE_RECEIVE)
    {
        ...
        if (p->state == 1)
        {
            ...
            if (data[status] != '2')
            {
                ...
                char *buf;
                data[eol] = '\0';
                while ((int64_t)eol > (int64_t)status &&
...
int proxy_socks5_negotiate(ProxySocket *p, int change)
{
    ...
    if (change == PROXY_CHANGE_RECEIVE)
    {
        ...
        if (p->state == 3)
        {
            ...
            if ((int64_t)bufchain_size(&p->pending_input_data) < (int64_t)len)
