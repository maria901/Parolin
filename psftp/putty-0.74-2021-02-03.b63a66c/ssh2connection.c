...
static bool ssh2_connection_filter_queue(struct ssh2_connection_state *s)
{
    ...
    while (1)
    {
        ...
        case SSH2_MSG_CHANNEL_FAILURE:
            ...
            expect_halfopen = (pktin->type == SSH2_MSG_CHANNEL_OPEN_CONFIRMATION ||
...
static void ssh2_set_window(struct ssh2_channel *c, int newwin)
{
     ...
     * window as well).
     */
    if ((s->ppl.remote_bugs & BUG_SSH2_MAXPKT) && (int64_t)newwin > (int64_t)OUR_V2_MAXPKT)
...
static void ssh2_sharing_globreq_response(__attribute__((unused)) struct ssh2_connection_state *s,
                                          PktIn *pktin,
                                          void *ctx)
{
