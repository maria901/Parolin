...
#define THE_COMPRESSOR_FUNCION_NAME_Z  compress_thread_2     /* the name of the compressor function */
#define THE_DECOMPRESSOR_FUNCION_NAME_Z decompress_thread_2
...
endlop:                 if (fcode.e.ent >= FIRST && rpos < rsize)
	...
	if ((int)fwrite_z(outbuf, 1, (outbits + 7) >> 3, fdout, NULL) != (int)(outbits + 7) >> 3)
	{
		amanda->internal_error_arp = 6;
		return 6; //
	}
