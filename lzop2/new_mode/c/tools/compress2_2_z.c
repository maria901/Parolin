 ...
 *
 */
char *valquiria_wide_to_utf8_no_oct_v2(WCHAR *pUSC2_maria)
{
...
wchar_t *
remove_permissive_name_m_no_october(wchar_t *wname)
{
...
uint32_t
tuklib_cpucores_z(void); //borrowed from XZ
...
saida:
     ...
     if (0 == ptr_my_struct_z->retvalue)
     {
          if (unicodemode)
          {
               ...
               ptr_my_struct_z->dest = _wfopen_z(permissive_name_m_v28(amanda_utf8towide_1_v28(temp_files_z[ptr_my_struct_z->thread_id_z], ar_temp), ar_temp2), "rb+", MAX_MEMORY_SIZE_Z, __FILE__, __LINE__, ptr_my_struct_z->dest);
          ...
          while (n_threads_copy--)
          {
               ...
               else
               {
                    if (unicodemode)
                    {
                         ...
                         ptr_my_struct_z->dest = _wfopen_z(permissive_name_m_v28(amanda_utf8towide_1_v28(temp_files_z[n_thread_counter], ar_temp), ar_temp2), "wb", MAX_MEMORY_SIZE_Z, __FILE__, __LINE__, NULL);
               ...
               ptr_my_struct_z->ar.string[3] = 0x53; //                        //signature for new lzop2 multi-thread

               pedro_dprintf(-1, "hex value %x\n", *((int *)ptr_my_struct_z->ar.string));
               ...
               if (0xFFFFFFFF == (unsigned int)(ptr_my_struct_z->ar.attrib))
               {
          ...
          for (i_z = 0; i_z < n_threads_z; i_z++)
          {
               ...
               if (dest)
               {
                    ...
                    if (unicodemode)
                    {
                         ...
                         temp_z = _wfopen_z(permissive_name_m_v28(amanda_utf8towide_1_v28(temp_files_z[i_z], ar_temp), ar_temp2), "rb", MAX_MEMORY_SIZE_Z, __FILE__, __LINE__, files_to_close_z[i_z]);
...
#error cannot occur...
          ...
          for (i_z = 0; i_z < n_threads_z; i_z++)
          {
               ...
               _wunlink(permissive_name_m_v28(
                   amanda_utf8towide_1_v28(temp_files_z[i_z], ar_temp), ar_temp2));
...
{
     return compress2_uncompress_k_mt_z(input, output);
}
...
int compress2_uncompress_k_mt_z(char *input, char *output)
{
	...
	first need to pass the number of threads and temp folder
...
init_position_z:;
     ...
     pedro_dprintf(-1, "single thread decompression time %.3f\n", inittimer2(1));
