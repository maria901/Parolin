...
int    lzop2compress (char *input, char *output, int levelin);
...
int __stdcall startapi (int parameter)
{
	...
	{
		intreturn = 0;
		intreturn = lzop2uncompress (myinfile, myoutfile);
	}
	if (4 == comando)
	{
		intreturn = 0;
		intreturn = lzop2compress_rsp32_k (myinfile, myoutfile, levelok);
	}
	intstatus = 0;
	return 0;
}
...
BOOL WINAPI DllMain (HINSTANCE hModule, DWORD ul_reason_for_call, LPVOID lpReserved)
{
	...
	case DLL_PROCESS_ATTACH:
		OutputDebugString ("BW Lzop2 DLL DLL_PROCESS_ATTACH (" __TIME__ " " __DATE__ ") _amanda_debug_ \n");
...
int __stdcall interface1 (__INT32_OR_INT64 argumento1, __INT32_OR_INT64 argumento2, __INT32_OR_INT64 argumento3, __INT32_OR_INT64 argumento4)
{
	...
	if (!strcmp ((char *) argumento1, "compressbzip3_rsp32_k"))
	{
		modo = 1;
		strcpy (inputfile, (char *) argumento2);
		strcpy (outputfile, (char *) argumento3);
...
int __stdcall About()
{
	mprintf2("Win32 and Win64 DLL to compress files using lzop2 compression methods");
	return 0;
}
...
int __stdcall Compresslzop2(char*Inputfile,char*OutPutFile,int CompressionLevel)
{
	char level[300];
	int ret;
	unicodemode=1;
...
int __stdcall CompressLzop2_rsp32_k(char*Inputfile,char*OutPutFile,int CompressionLevel)
{
	char level[300];
	int ret;
	unicodemode=1;
	...
	*/
	...
	ret = interface1((__INT32_OR_INT64)"compressbzip3_rsp32_k",(__INT32_OR_INT64)Inputfile, (__INT32_OR_INT64)OutPutFile, (__INT32_OR_INT64)level);
...
int __stdcall DecompressLzop2(char*Inputfile,char*OutPutFile)
{
	int ret;
	unicodemode = 1;
	ret = interface1((__INT32_OR_INT64)"status", 0, 0, 0);
...
*
*/
int __stdcall compress_l2___rspk_ar(char * Input_amanda_file, char * OutPut_ricardo_File, int Compression_juliete_Level, int n_threads_z__, char *temp_path_z__)
{
	pedro_dprintf(-1, "dentro\n");
	is_multi_thread_z = false         ;
	n_threads_z = n_threads_z__       ;	
	strcpy(temp_path_z, temp_path_z__);
	return CompressLzop2_rsp32_k(Input_amanda_file, OutPut_ricardo_File, Compression_juliete_Level);
}
/**
*
...
*
*/
int __stdcall uncompress_l2___rspk_ar(char * Inputfile_arp, char * OutPutFile_arp)
{
	is_multi_thread_z = false;
	return DecompressLzop2(Inputfile_arp, OutPutFile_arp);
}

int __stdcall get_progress_l2___ar(void)
{
	return GetProgress();
}

int __stdcall pause_l2___aakp_l2___ar(void)
{
	PauseCompress();
	return 0; 
}

int __stdcall resume_l2___aakp_l2___ar(void)
{
	ResumeCompress();
	return 0;
}

int __stdcall cancel_l2___aakp_l2___ar(void)
{
	CancelCompress();
	return 0;
}
