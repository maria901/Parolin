//sligthtly modified by the 'guy' in 2021...
...
int
    decompress(fdin, fdout) int fdin;
int fdout;
{
                                    ...
                                    __attribute__((unused)) int Compression_juliete_Level_unused___)
{
      ...
      fd_aakp = _wopen(amanda_utf8towide_1_v27(Input_amanda_file_utf_8), O_RDONLY | O_BINARY, _S_IREAD);

      if (-1 != fd_aakp)
      {
            fd_valquiria_aakp = _wopen(amanda_utf8towide_1_v27(OutPut_ricardo_File_utf_8), O_WRONLY | O_CREAT | O_BINARY, _S_IWRITE);

            pedro_dprintf(-1, "retorno de open de output %d file %s\n", fd_valquiria_aakp, OutPut_ricardo_File_utf_8);

            if (-1 != fd_valquiria_aakp)
            {
...
saida_amanda_agora:;
      ...
      if (-1 != fd_aakp)
      {
            if (-1 == close(fd_aakp))
            {
                  return_value_valquiria = 3; // cannot close the input file
            }
            fd_aakp = 0;
      }
      if (-1 != fd_valquiria_aakp)
      {
            if (-1 == close(fd_valquiria_aakp))
            {
                  return_value_valquiria = 4; //cannot close output file
            }
            fd_valquiria_aakp = 0;
      }
...
int __stdcall uncompress_co___rspk_ar(char *Input_amanda_file_utf_8, char *OutPut_ricardo_File_utf_8)
{
      ...
      processado_rspk = 0;
      porcentagem_junior_ale_mislaine_thalia_aline_juliete_valquiria_maria_feline_pedro = 0;
      fd_aakp = _wopen(amanda_utf8towide_1_v27(Input_amanda_file_utf_8), O_RDONLY | O_BINARY);
...
#ifdef TEST_AMANDA_RICARDO__
int main()
{
      int ret_val_juliete = 0;
