									...
									__attribute__((unused)) int Compression_juliete_Level_unused___)
{
	...
	fd_aakp = _wopen(utf8towide(Input_amanda_file_utf_8), O_RDONLY|O_BINARY, _S_IREAD);

	if(-1 != fd_aakp)
	{
		fd_valquiria_aakp = _wopen(utf8towide(OutPut_ricardo_File_utf_8), O_WRONLY|O_CREAT|O_BINARY, _S_IWRITE);
...
int __stdcall uncompress_co___rspk_ar(char * Input_amanda_file_utf_8, char * OutPut_ricardo_File_utf_8)
{
	...
	processado_rspk =                                                         0;
	porcentagem_junior_ale_mislaine_thalia_aline_juliete_valquiria_maria_feline_pedro = 0;
	fd_aakp = _wopen(utf8towide(Input_amanda_file_utf_8), O_RDONLY|O_BINARY);
