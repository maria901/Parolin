                                                                   ...
                                                                   const ZSTD_compressionParameters *const cParams)
{
    ...
    if (kHasSIMD128)
    {
        if (cParams->windowLog > 14)
            mode = ZSTD_urm_enableRowMatchFinder;
    }
    else
    {
    ...
    const U64 pledgedSrcSize)
{
    ...
    size_t const ldmSeqSpace = ldmParams->enableLdm ? ZSTD_cwksp_aligned_alloc_size(maxNbLdmSeq * sizeof(rawSeq)) : 0;
...
//#define ZSTD_MULTITHREAD
    ...
    if (cctx->appliedParams.nbWorkers > 0)
    {
        ...
        for (;;)
        {
            ...
            pedro_dprintf(-1, "size %lld", (int64_t)cctx->consumedSrcSize);

            //processado_rspk += (U64)(input->pos - ipos);
