...
static __int64 tamanho____aakp;
...
//dl--
               ...
               || (directive == ZSTD_e_end && stillToFlush != 0))
        {
            ...
            pedro_dprintf(-1, "ZSTD_compress_generic(end:%u) => input pos(%u)<=(%u)size ; output generated %u bytes \n",
                                        ...
                                        int compressionLevel)
{
    ...
    if (ress.dstFile == NULL)
    {
        ...
        if (strcmp(srcFileName, stdinmark) && UTIL_stat(srcFileName, &statbuf) && UTIL_isRegularFileStat(&statbuf))
        {
                                ...
                                const char *dstFileName, const char *srcFileName)
{
    ...
    for (;;)
    {
        ...
        else if ((prefs->overwrite) && !strcmp(dstFileName, stdoutmark))
        { /* pass-through mode */
                                 ...
                                 const char *dstFileName, const char *srcFileName)
{
    ...
    if ((ress.dstFile == NULL) && (prefs->testMode == 0))
    {
        ...
        if (strcmp(srcFileName, stdinmark) /* special case : don't transfer permissions from stdin */
            && UTIL_stat(srcFileName, &statbuf) && UTIL_isRegularFileStat(&statbuf))
        {
            dstFilePermissions = statbuf.st_mode;
        }
