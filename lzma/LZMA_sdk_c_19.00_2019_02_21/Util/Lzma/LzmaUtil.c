...
extern int lgetpor_ar (__int64 max, __int64 fatia);
...
{
	__int64 fatia_ar           = 0;
	extern int porcentagem_ar;
	extern __int64 file_tamanho_ar;
	extern int intcancel_ar;
	extern int intpause_ar;
	...
	for (;;)
	{
		...
		porcentagem_ar = lgetpor_ar (file_tamanho_ar, fatia_ar);
...
int main_ar(int numArgs, const char *args[])
{
	char rs[800] = { 0 };
	...
	if(0 == strcmp("e", args[1]))
	{
		...
		if (SZ_OK != res)
		{
			if (SZ_ERROR_MEM == res )
			{
				return 3;       //SZ_ERROR_MEM
			}
			else if (SZ_ERROR_DATA == res)
			{
				return 4;       //SZ_ERROR_DATA
			}
			else
			{
				return 5;       //Unexpected
